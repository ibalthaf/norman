		...
		<div class="container">
			...
			<div class="handover_form">
				...
				//check date
					if(empty($_GET["to"])):
					//select data
						$sql	= "SELECT @count:=@count+1 AS sno, SITE.`site_name` AS sitename, SITE.`site_owner_name` AS ownername, SITE.`site_plot_no` AS plotno, REPORT.*, DATE_FORMAT(REPORT.hform_createdon, '%d/%m/%Y') AS createdon, PDF.`pdf_name` AS pdfname FROM (SELECT @count:=0) AS serialno, `nman_handover_certificate` AS REPORT INNER JOIN `nman_site_master` AS SITE ON REPORT.`fk_site_id`=SITE.`pk_site_id` LEFT JOIN `nman_pdf_master` AS PDF ON PDF.`pdf_key`=REPORT.`pdf_key` WHERE SITE.`pk_site_id`=:pk_site_id AND SITE.`site_visibility`=:site_visibility";
					//prepare statement
						$stmt = $db->prepare($sql);
