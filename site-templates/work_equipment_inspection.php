...
<html lang="en">
	<head>
		...
		<!-- themestyle -->
		<link href="css/report-templates/work_equipment_inspection.css" type="text/css" rel="stylesheet">
	</head>
	<body>
		<div class="container">
			...
			<div class="FormTitle">
				<h5 style="color:#000;    font-size: large;"><strong><b>WORK EQUIPMENT INSPECTION FORM</b></strong></h5>
			</div>
			<div class="handover_form">
				...
				//check date
					if(empty($_GET["to"])):
					//select data
						$sql	= "SELECT @count:=@count+1 AS sno, REPORT.*, SITES.`site_name` AS sitename, SITES.`site_owner_name` AS ownername, SITES.`site_plot_no` AS plotno, DATE_FORMAT(REPORT.wei_createdon, '%d/%m/%Y') AS createdon, CONCAT(USERS.`user_first_name`, ' ', USERS.`user_last_name`) AS inpectedby, PDF.`pdf_name` AS pdfname FROM (SELECT @count:=0) AS serialno, `nman_site_master` AS SITES INNER JOIN `nman_work_equipment_inspection` AS REPORT ON REPORT.`fk_site_id`=SITES.`pk_site_id` INNER JOIN `nman_user_master` AS USERS ON USERS.`pk_user_id`=REPORT.`fk_user_id` LEFT JOIN `nman_pdf_master` AS PDF ON PDF.`pdf_key`=REPORT.`pdf_key` WHERE SITES.`pk_site_id`=:pk_site_id AND SITES.`site_visibility`=:site_visibility GROUP BY REPORT.`pdf_key`";
					//prepare statement
						$stmt = $db->prepare($sql);
					...
					//select data
						$sql	= "SELECT @count:=@count+1 AS sno, REPORT.*, SITES.`site_name` AS sitename, SITES.`site_owner_name` AS ownername, SITES.`site_plot_no` AS plotno, DATE_FORMAT(REPORT.wei_createdon, '%d/%m/%Y') AS createdon, CONCAT(USERS.`user_first_name`, ' ', USERS.`user_last_name`) AS inpectedby, PDF.`pdf_name` AS pdfname FROM (SELECT @count:=0) AS serialno, `nman_site_master` AS SITES INNER JOIN `nman_work_equipment_inspection` AS REPORT ON REPORT.`fk_site_id`=SITES.`pk_site_id` AND DATE(REPORT.`wei_createdon`)>=:startdate AND DATE(REPORT.`wei_createdon`)<=:enddate INNER JOIN `nman_user_master` AS USERS ON USERS.`pk_user_id`=REPORT.`fk_user_id` LEFT JOIN `nman_pdf_master` AS PDF ON PDF.`pdf_key`=REPORT.`pdf_key` WHERE SITES.`pk_site_id`=:pk_site_id AND SITES.`site_visibility`=:site_visibility GROUP BY REPORT.`pdf_key`";
					//prepare statement
						$stmt = $db->prepare($sql);
				...
				<table class="w-100" cellpadding="5px">
					<thead class="height">
						<tr>
							...
							<th class="w-25 cls_th text-center">Inspection Completed By</th>
						</tr>
					</thead>
					<tbody>
						<?php
