...
//fetch data
	...
	$dwsdata = "SELECT * FROM `nman_daywork_sheet_form` WHERE `fk_site_id`=:fk_site_id AND `fk_user_id`=:fk_user_id AND `work_type`=:work_type AND `dform_visibility`=1 ORDER BY pk_dform_id DESC";
//execute query
	$sltmst	= $db->prepare($dwsdata);
...
<html lang="en">
	<head>
		...
		<link href="css/brickwork/daywork-sheet/style.css" type="text/css" rel="stylesheet">
	</head>
	<body>
		<div id="footer">
			<p class="page text-right">DAYWORK SHEET</p>
		</div>
		...
		<div class="managerTitle">
			<h5 style="color:#fff;"><strong>DAYWORK SHEET</strong></h5>
		</div>
		<table style="width:100%" class="arialTable">
			<tbody>
				<tr>
					...
					<td class="w-15" style="padding-left: 80px !important;">Date</td>
					<td class="w-10" >
		...
		<table style="width:100%" class="arialTable">
			<tbody>
				<tr>
					...
					<td class="w-15" style="padding-left: 80px !important;">Daywork Reference </td>
					<td class="w-10">
						<div class="input2"><?php echo $fetchmst["daywork_ref"]; ?></div>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<div style="background-color:#d9d9d9; padding:1%; border-radius:40px; margin-top: 4px;">
			<table  style="width:100%" class="arialTable">
				<tbody>
					<tr>
						<td class="w-30" style="line-height: 20px;">Plot</td>
						<td class="w-20" style="">
							<div class="customtd" style="width: 15%;"><?php echo $fetchmst["input_1"]; ?></div>	
						</td>
					</tr>
					<tr>
						<td class="w-30" style="line-height: 20px;">Relavent Drawing Numbers</td>
						<td class="w-20" style="">
							<div class="customtd" style="width: 15%;"><?php echo $fetchmst["input_2"]; ?></div>	
						</td>
					</tr>
					<tr>
			...
			<table style="width:100%" >
				<tbody>
					<tr>
						<td class="w-100">
							<div class="customtd" style="height:230px; padding: 20px !important;"><?php echo $fetchmst["input_3"]; ?>							</div>	
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="managerTitle">
			<h5 style="color:#fff;"><strong>SITE REPRESENTATIVE</strong></h5>
		</div>
		<div style="background-color:#d9d9d9">
			<table  style="width:100%" class="arialTable">
				<tbody>
					...
					<td class="w-20">
						<div class="customtd"><?php echo $fetchmst["site_representative"]; ?></div>	
					</td>
					<td class=" text-center w-5">Signature</td>
					<td class="w-20">
						<div class="customtd text-center" style="background:#fff;">
							<img src="<?php echo "data:image/png;base64,".$fetchmst["site_signature"]; ?>" width="40px" height="25px" />
							</div>		
					</td>
				</tbody>
			</table>
		...
		<div class="managerTitle">
			<h5 style="color:#fff;"><strong>NORMAN BRICKWORK SITE REPRESENTATIVE</strong></h5>
		</div>
		<div style="background-color:#d9d9d9">
			<table  style="width:100%" class="arialTable">
				<tbody>
					...
					<td class="w-20">
						<div class="customtd text-center" style="background:#fff;">
							<img src="<?php echo "data:image/png;base64,".$fetchmst["user_signature"]; ?>" width="40px" height="25px" />
							</div>	
					</td>
				</tbody>
			</table>
		...
		<?php
		//select form fields
			$sltimg 	= "SELECT * FROM `nman_daywork_sheet_images` WHERE `fk_dsform_id`=:fk_dsform_id  AND `image_visibility`=1";
			$sltimages	= $db->prepare($sltimg);
				$sltimages->bindParam("fk_dsform_id"	, $fetchmst["pk_dform_id"]);
		...
		//fetch data
			...
			if($sltimages->rowCount() > 0):
				...
				foreach($imageData as $key=>$img):
					echo "<td class='text-center'><img src='".$img["dsf_image"]."' /></td>";
