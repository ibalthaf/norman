...
//get registered user list
	var gethandoverreport = function(txtDateFrom, txtDateTo){
	//show loader
		$(".ajax-loader").show();
	//JSON data
		var data ={
	...
	//server action
        service.operationDataService('get_handover_report', JSON.stringify(data), function(data){
		//prepare datatable
			$('#hand-over-table').dataTable( {
				data: data.data,
				columns: [
					{
						'data'	: null,
	...
	//from date
		...
		//end date
			...
			//sort data
				...
				gethandoverreport($("#txtDateFrom").val(), date.format('Y-m-d'));
			});
		});
	//get data
		gethandoverreport("", "");
