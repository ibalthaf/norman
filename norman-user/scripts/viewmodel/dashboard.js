...
//click event
	...
	//server action
        service.operationDataService('get_form_info', JSON.stringify(data), function(data){
			...
			$(".lblStatus").html("").html((data.data.status==1 ? '<button class="btn btn-success btn-xs" type="button">Active</button>' : '<button class="btn btn-danger btn-xs" type="button">InActive</button>'));
		});
	});
